---
name: gog
description: "gog CLI: safe Google Workspace automation, JSON, auth, scoped reads/writes. Use for Google Workspace work from the shell — Gmail, Calendar, Drive, Docs, Sheets, Contacts, Tasks, and more — when built-in Google connectors are missing a feature or automation needs stable JSON."
metadata:
    author: openclaw
    github-path: .agents/skills/gog
    github-ref: refs/tags/v0.35.0
    github-repo: https://github.com/openclaw/gogcli
    github-tree-sha: ecf231ea3ba19e11c8f3f9d69971776ab308b5d0
---

# gog

> **Vendored from `openclaw/gogcli` at `v0.35.0`.** Local edits are confined to this banner and the frontmatter — `metadata.*` records where refreshes come from, and a trigger sentence is appended to `description` so the skill fires on Workspace-from-the-shell requests. Everything below this banner is upstream's, unmodified.
>
> **Refreshing: diff against the recorded ref, do not work from memory.**
>
> ```
> gh api "repos/openclaw/gogcli/contents/.agents/skills/gog/SKILL.md?ref=v0.35.0" \
>   -H "Accept: application/vnd.github.raw" > /tmp/upstream.md
> diff /tmp/upstream.md skills/gog/SKILL.md
> ```
>
> Upstream ships this umbrella skill alongside per-service skills (`gog-gmail`, `gog-calendar`, …) generated from `gog schema --json`, and hand-authored workflow skills (`gog-inbox-triage`, `gog-meeting-prep`, …). Its own advice is to start with `gog` alone and add a service or workflow skill only when the agent needs it — so those stay upstream until one earns its place here.
