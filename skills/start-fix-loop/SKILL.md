...
3. Record each head SHA, and baseline **only the already-resolved threads**, so settled findings are not re-litigated. Never baseline an unresolved one. Resolved means somebody decided; unresolved means outstanding work, whether it arrived a second ago or before the loop existed. A baseline of "every thread that exists right now" silently swallows every finding already waiting — the loop then runs perfectly and fixes nothing.
