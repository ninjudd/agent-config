...
**A plan whose frontmatter says `status: Draft` is exempt as a whole.** The shared convention in `AGENTS.md` fronts every project document with a `status:` keyword, and Draft is the one that declares implementation has not started and the plan is not yet claiming to be executable. The gate above exists to stop a plan from asserting a readiness it does not have; a Draft plan asserts none, so its open questions withhold nothing — owned or not — and the verdict comes from the rest of the review alone. Still name the open questions in the review body, and say that Draft is why they do not block: the exemption is the frontmatter's doing, and an approval that names it cannot be misread as the gate having been forgotten. A document with no `status:` frontmatter gets no exemption — absence of a claim and a declared not-ready are different things, and only the declaration is load-bearing.

The claim the gate protects does not disappear with Draft; it moves to the pull request that flips the plan out of it. That flip is the readiness assertion, so every question still open at that moment is the flipping pull request's to answer for — answered, or deferred under the exception above — exactly as if it had introduced them. Without that, the exemption is a bypass in two commits: land the plan as Draft, then flip the keyword in a one-line pull request that nothing blocks. Review the flip against the whole plan's questions, not against the one-line diff.

**A pull request that creates a plan is not a special case, and following the rule through shows why it should not be one.** When the pull request *is* the plan, every question in it is one the pull request introduces, so all of them are in scope, and a new plan that arrives claiming readiness — any status but Draft — cannot be approved with bare questions. That is the intended outcome rather than an artefact of the wording. Readiness as a whole is gated at exactly two points: creation, when the plan arrives asserting it, and the flip out of Draft, when it deferred the assertion; between and after those, the same questions are pre-existing and nothing blocks on them again. A new plan therefore has two honest ways to land, and they say different things: mark the questions it means to settle by building — the deferral exception doing its job — or arrive as Draft, which defers the whole claim rather than any single question.
